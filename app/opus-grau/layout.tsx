...
export default function OpusGrauLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-[#ECECEA] text-[#0B0B0D] relative overflow-x-hidden" style={{ fontFamily: "Fraunces, serif" }}>
      {/* Soft color orbs in background — gedämpft auf neutralem Grau */}
      <div className="pointer-events-none fixed inset-0 z-0 opacity-25">
        <div className="absolute -top-40 -right-40 w-[50vw] h-[50vw] blur-[120px]" style={{ background: "radial-gradient(circle, #F7C80044 0%, transparent 70%)" }} />
        <div className="absolute top-[40%] -left-40 w-[40vw] h-[40vw] blur-[120px]" style={{ background: "radial-gradient(circle, #E23A2F33 0%, transparent 70%)" }} />
        <div className="absolute bottom-[10%] right-[20%] w-[30vw] h-[30vw] blur-[120px]" style={{ background: "radial-gradient(circle, #1F49B822 0%, transparent 70%)" }} />
      </div>
      ...
      <header className="relative z-10 border-b border-[#0B0B0D]/15">
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 py-6 flex items-center justify-between gap-6">
          <Link href="/opus-grau" className="flex items-center gap-4">
          ...
          <nav className="hidden md:flex items-center gap-6 text-[14px]" style={{ fontFamily: "Inter, sans-serif" }}>
            {nav.slice(1).map((item, i) => {
              ...
              return (
                <Link key={item.href} href={`/opus-grau${item.href}`} className={`relative tracking-wide transition-colors ${hoverColor} group`}>
                  {item.label}
          ...
          <Link
            href="/opus-grau/termine"
            className="hidden md:inline-flex items-center gap-2 text-sm px-5 py-3 bg-[#0B0B0D] text-[#ECECEA] hover:bg-jko-red transition-colors relative group"
          ...
          <div style={{ fontFamily: "Inter, sans-serif" }}>
            ...
            <ul className="space-y-1.5 text-sm">
              {nav.slice(1).map((item) => (
                <li key={item.href}><Link href={`/opus-grau${item.href}`} className="hover:text-jko-red">{item.label}</Link></li>
              ))}
            </ul>
          </div>
          <div style={{ fontFamily: "Inter, sans-serif" }}>
...
kontakt@jko-tauberfranken.de</p>
        ...
        <div className="border-t border-[#0B0B0D]/10 py-4 text-center text-[11px] text-[#0B0B0D]/40" style={{ fontFamily: "Inter, sans-serif" }}>
          © {new Date().getFullYear()} JKO Tauberfranken · Prototyp · Opus Grau
        </div>
      </footer>
    </div>
  );
